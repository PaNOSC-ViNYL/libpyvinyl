...
class BaseFormat(AbstractBaseClass):
    ...
    @classmethod
    @abstractmethod
    def convert(self, input: str, output: str, output_format_class: str, key, **kwargs):
...
class ExampleAFormat(BaseFormat):
    ...
    @classmethod
    def convert(
        cls, input: str, output: str, output_format_class: str, key=None, **kwargs
    ):
        """Direct convert method, if the default converting would be too slow or not suitable for the output_format"""
        if output_format_class is ExampleBFormat:
            cls.convert_to_ExampleBFormat(input, output)
        else:
            raise TypeError("Unsupported format {}".format(output_format_class))
        if key is not None:
...
class ExampleBFormat(BaseFormat):
    ...
    @classmethod
    def convert(cls, input: str, output: str, output_format_class: str, key, **kwargs):
        """Direct convert method, if the default converting would be too slow or not suitable for the output_format"""
        raise NotImplementedError
...
if __name__ == "__main__":
    ...
    test1_convert = test1_dataA.write(
        "data_convert.npy", ExampleBFormat, key="test_convert"
    )
